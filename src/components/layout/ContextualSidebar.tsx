...
export function ContextualSidebar({
  ...
  const activeItem = resolveRouteMatch(pathname, items);
  ...
  return (
    <>
      <aside className={`hidden w-[248px] shrink-0 border-r px-4 py-5 lg:block ${panelClass}`}>
        ...
        <nav className="mt-4 space-y-1" aria-label={label}>
          {items.map((item) => {
      ...
      <section className={`border-b px-4 py-3 lg:hidden ${panelClass}`} aria-label={`${label} section menu`}>
        <p className={`text-[10px] font-extrabold uppercase tracking-[0.08em] ${eyebrowClass}`}>
          {label}
        </p>
        <nav className="mt-3 flex gap-2 overflow-x-auto pb-1" aria-label={label}>
          {items.map((item) => {
            const Icon = item.icon;
            const isActive = activeItem?.href === item.href;
