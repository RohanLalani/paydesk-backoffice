            ...
            <table className="min-w-[1180px] w-full text-left text-sm">
              <thead className={`border-b text-xs uppercase ${isDark ? "border-slate-400/15 text-slate-400" : "border-[#ded8f3] bg-[#fbfaff] text-slate-500"}`}>
                <tr>
                  ...
                  <th className="px-4 py-3">{sortableHeader("Field Changed", "fieldChanged")}</th>
              ...
              <tbody className="divide-y divide-slate-400/10">
                {rows.map((row) => (
                  <tr key={row.id} onClick={() => setSelectedRow(row)} className={`cursor-pointer transition ${selectedRow?.id === row.id ? (isDark ? "bg-white/[0.06]" : "bg-[#f3f0ff]") : isDark ? "hover:bg-white/[0.04]" : "hover:bg-[#fbfaff]"}`}>
                    ...
                    <td className="px-4 py-3 font-bold">{row.fieldChanged}</td>
                    <td className="max-w-[180px] px-4 py-3 font-semibold"><span className="block truncate">{formatValue(row.previousValue, row.fieldKey)}</span></td>
                    <td className="max-w-[180px] px-4 py-3 font-semibold"><span className="block truncate">{formatValue(row.newValue, row.fieldKey)}</span></td>
                ...
                {!isLoading && rows.length === 0 ? (
                  <tr><td colSpan={11} className={`px-4 py-10 text-center text-sm font-bold ${muted}`}>No product logs found.</td></tr>
                ) : null}
                {isLoading ? (
                  <tr><td colSpan={11} className={`px-4 py-10 text-center text-sm font-bold ${muted}`}>Loading product logs...</td></tr>
                ) : null}
              </tbody>
            </table>
          </div>
        ...
        <aside className={`rounded-[8px] border p-5 ${panel}`}>
          {selectedRow ? (
            <div className="space-y-4">
              <div>
                <h2 className="text-lg font-extrabold tracking-normal">{selectedRow.fieldChanged}</h2>
              ...
              <div className="grid gap-3">
                <div className={`rounded-[8px] border p-3 ${subtlePanel}`}>
                  <h3 className="text-xs font-extrabold uppercase tracking-normal">Previous Value</h3>
                  <p className="mt-2 break-words text-sm font-semibold">{formatValue(selectedRow.previousValue, selectedRow.fieldKey)}</p>
                </div>
                <div className={`rounded-[8px] border p-3 ${subtlePanel}`}>
                  <h3 className="text-xs font-extrabold uppercase tracking-normal">New Value</h3>
                  <p className="mt-2 break-words text-sm font-semibold">{formatValue(selectedRow.newValue, selectedRow.fieldKey)}</p>
                </div>
              </div>
            </div>
          ) : (
